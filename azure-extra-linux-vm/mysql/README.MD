...
Note! When using Telegraf’s default JSON parser (with data_format = "json"), it automatically extracts numeric, boolean, and time values but ignores string values unless you explicitly tell it to keep them.

How to Include String Fields:

You can modify your input configuration to tell Telegraf to retain specific string fields by using the json_string_fields option. 
...
[[inputs.file]]
  ...
  data_format = "json"
  json_string_fields = ["tag1_quality", "tag2_quality"]
...
{"measurement": "sensor", "tags": {"tag": "tag1"}, "fields": {"value": 101, "active": 1, "state": 0, "quality": "good"}}
{"measurement": "sensor", "tags": {"tag": "tag2"}, "fields": {"value": 55, "active": 1, "state": 0, "quality": "good"}}

```

## Install Telegraf Ubuntu and make ETL  input file 2 output file
...
```json
{"fields":{"tag1_active":1,"tag1_quality":"good","tag1_state":0,"tag1_value":101,"tag2_active":1,"tag2_quality":"good","tag2_state":0,"tag2_value":55},"name":"file","tags":{"host":"vmdocker01"},"timestamp":1740516350}
{"fields":{"tag1_active":1,"tag1_quality":"good","tag1_state":0,"tag1_value":101,"tag2_active":1,"tag2_quality":"good","tag2_state":0,"tag2_value":55},"name":"file","tags":{"host":"vmdocker01"},"timestamp":1740516375}
...
## Use Telegraf and keep the same input file and output MySql 

We need the output mysql plugin, check what we already have:
...
# signalfx
# socket_writer
  sql # ms sql, mysql, pgx, snowflake etc
# stackdriver
# stomp
...
Output MySql: This was pain.......must read more. The plugin was hard, always "you have an error"

* https://github.com/influxdata/telegraf/blob/master/plugins/outputs/sql/README.md


Lets create a telegraf database and the user
...
create user 'telegraf_user'@'localhost' identified by 'etlaccount1';

grant all privileges on telegraf_db.* to 'telegraf_user'@'localhost';

flush privileges;

exit

# login is as user and give pass

mysql -u telegraf_user -p

# login with params

mysql --user=telegraf_user --password=etlaccount1
...
Loading config: telegraf.conf
> file,host=vmdocker01 tag1_active=1,tag1_quality="good",tag1_state=0,tag1_value=101,tag2_active=1,tag2_quality="good",tag2_state=0,tag2_value=55 1740602614000000000

```

Run telegraf with the configiration

```bash
sudo telegraf --config telegraf.conf

# or just start it since config test is ok
